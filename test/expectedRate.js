...
let MartletInstantlyTrader = artifacts.require("./MartletInstantlyTrader.sol");
...
contract('ExpectedRates', function(accounts) {
    it("should init  martletInstantlyTrader and all its components.", async function () {
        ...
        martletInstantlyTrader = await MartletInstantlyTrader.new(admin, {});
        var events = martletInstantlyTrader.allEvents(['ListSupplierPairs']);
        ...
        reserve1 = await Supplier.new(martletInstantlyTrader.address, pricing1.address, admin);
        ...
        // add reserves
        await martletInstantlyTrader.addSupplier(reserve1.address, true);

        //set contracts
        let kgtToken = await TestToken.new(" genesis token", "KGT", 0);
        ...
        expectedRates = await ExpectedRate.new(martletInstantlyTrader.address, admin);
        await martletInstantlyTrader.setParams(whiteList.address, expectedRates.address, gasPrice.valueOf(), 15);
        await martletInstantlyTrader.setEnable(true);
        let price = await martletInstantlyTrader.maxGasPrice();
        assert.equal(price.valueOf(), gasPrice.valueOf());

        //list tokens per reserve
        for (let i = 0; i < numTokens; i++) {
            console.log("list tokens per reserve:", i, reserve1.address, ethAddress, tokenAdd[i], true);
            await martletInstantlyTrader.listPairForSupplier(reserve1.address, ethAddress, tokenAdd[i], true, {from: admin});
            await martletInstantlyTrader.listPairForSupplier(reserve1.address, tokenAdd[i], ethAddress, true, {from: admin});
        }
    });
    ...
    it("should test can't init expected rate with empty contracts (address 0).", async function () {
        ...
        try {
            expectedRateT =  await ExpectedRate.new(martletInstantlyTrader.address, 0);
        ...
        expectedRateT =  await ExpectedRate.new(martletInstantlyTrader.address, admin);
    });
    ...
    it("should test eth to token. use qty slippage.", async function() {
        ...
        let myExpectedRate = await martletInstantlyTrader.findBestRate(ethAddress, tokenAdd[tokenInd], qty);
        let qtySlippageRate = await martletInstantlyTrader.findBestRate(ethAddress, tokenAdd[tokenInd], (qty * quantityFactor));
    ...
    it("should test token to eth. use slippage.", async function() {
        ...
        let myExpectedRate = await martletInstantlyTrader.findBestRate(tokenAdd[tokenInd], ethAddress, qty);
        let qtySlippageRate = await martletInstantlyTrader.findBestRate(tokenAdd[tokenInd], ethAddress, (qty * quantityFactor));
